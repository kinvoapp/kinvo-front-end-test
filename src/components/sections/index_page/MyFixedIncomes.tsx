...
const InternalGrid = styled.div.attrs((props) => ({
  ...
  grid-template-columns: ${(props) =>
    props.templateColumns || `repeat(${props.numColumns}, 1fr)`};
  grid-template-rows: 1fr;
`;

function OutlinedInfo(props: {
  title: string;
  info: {
    name?: string;
    value: string | number;
    ...
    <OutlinedInfoWrapper>
      <Flex direction="column" width="100%">
        ...
        <InternalGrid
          ...
          templateColumns={props.gridTemplateColumns}
        >
          {info.map(
            (
              { name, value, color = "text", shade = "main" },
              key,
            ) => (
              <Flex grow direction="column" key={key}>
                {name ? (
                  <Text variant="card_subtitle">{name}</Text>
                ) : null}
                <Text
                  selectable
                  variant="card_info_3"
...
const ExternalGrid = styled.div.attrs((props) => ({
  ...
  background-color: ${(props) =>
    props.dark ? getTheme(props).background.lighter : "transparent"};
`;

function FixedIncomeBox(props: {
  titleInfo: FixedIncomeTitle;
  darkBg: boolean;
}) {
      ...
      <ExternalGrid dark={props.darkBg}>
        ...
        <OutlinedInfo
          ...
          info={[
            {
              name: "Valor inves.",
              value: formatNumber(position.valueApplied, "$"),
              color: "success",
            },
            {
              name: "Saldo bruto",
              value: formatNumber(position.equity, "$"),
              color: "success",
            },
            {
              name: "Rent.",
              value: formatNumber(position.profitability, "%"),
              color: "success",
            },
            {
              name: "% da cart.",
              value: formatNumber(position.portfolioPercentage, "%"),
              color: "success",
            },
            {
              name: position.indexerLabel,
              value: formatNumber(position.indexerValue, "$"),
              color: "success",
            },
            {
              name: `Sobre ${position.indexerLabel}`,
              value: formatNumber(
...
export function MyFixedIncomes() {
  ...
  const [filtered, setFiltered] = useState(
    fixedIncomeInfo?.data?.snapshotByProduct,
  );
  ...
  const numPages = filtered
    ? Math.ceil(filtered.length / INCOMES_PER_PAGE)
    : 1;
  ...
  useEffect(() => {
    ...
    if (query) {
      ...
      newFiltered =
        fixedIncomeInfo?.data?.snapshotByProduct.filter((info) =>
          regex.test(info.fixedIncome.name),
        ) ?? [];
    }
    ...
    switch (sorting.criteria) {
      case "title":
        newFiltered.sort(
          (a, b) =>
            a.fixedIncome.name.localeCompare(
              b.fixedIncome.name,
              "pt",
            ) * coefficient,
        );
      ...
      case "bond_type":
        newFiltered.sort(
          (a, b) =>
            a.fixedIncome.bondType.localeCompare(
              b.fixedIncome.bondType,
              "pt",
            ) * coefficient,
        );
        break;
      case "equity":
        newFiltered.sort(
          (a, b) =>
            (a.position.equity - b.position.equity) * coefficient,
        );
        break;
      case "value_applied":
        newFiltered.sort(
      ...
      case "profitability":
        newFiltered.sort(
          (a, b) =>
            (a.position.profitability - b.position.profitability) *
            coefficient,
        );
        break;
      case "due_date":
      ...
      <Card noPadding>
        ...
        >
          <Flex justify="space-between" align="center" padding={1}>
            <Text variant="card_title">Minhas Rendas Fixas</Text>
            <Flex gap={1}>
              <Select
                onChange={({ target }) =>
                  setSorting(JSON.parse((target as any).value))
                }
              >
                ...
                {Object.keys(sortingOptions).map((label, key) => {
                  ...
                  return (
                    <option
                      ...
                      selected={sorting.criteria === value.criteria}
                    >
                      {label}
                    </option>
                  );
              ...
              <Input
                ...
                value={query}
                onChange={({ target }) =>
                  setQuery((target as any).value)
                }
                startAdornment={<Icon src={searchIcon} />}
              />
            </Flex>
          </Flex>
          <Divider />
          <Flex grow width="100%" direction="column">
            {filtered
              ?.filter(
                (titleInfo, index) =>
                  index >= (page - 1) * INCOMES_PER_PAGE &&
                  index < page * INCOMES_PER_PAGE,
              )
              .map((titleInfo, index) => (
                <FixedIncomeBox
                  key={index}
                  titleInfo={titleInfo}
                  darkBg={index % 2 ? true : false}
                />
              ))}
          </Flex>
